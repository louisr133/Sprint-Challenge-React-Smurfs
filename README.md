...
- [x] Explain the differences between `client-side routing` and `server-side routing`.
  One of the main differences between the two is how it effects the browser. One refreshed the whole page ( server-side) and one updates the page without refreshing it (client-side)
- [s] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
  C - POST
  R - GET
  U - PUT
  D - DELETE
